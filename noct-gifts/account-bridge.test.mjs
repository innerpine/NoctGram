...
void test('client accepts new limits and preserves probabilities from older operation receipts',async t=>{
  for(const chance of [1,2,85,95]){
    const f=fixture();t.after(()=>f.dispose());await f.reply(linked());
    f.app.go({screen:'upgrade',fromUid:'canonical-1',fromId:'toy_bear',toId:'watch'});
    const pending=f.app.runUpgrade(),body=f.calls.at(-1).body,response=upgradeResult(body);
    response.operation.chance=chance;response.operation.roll=0;
    await f.reply(response);await pending;
    assert.equal(f.app.state.gameRetry,false);assert.equal(f.app.state.dialChance,chance);
    assert.equal(f.app.state.balance,500);assert.ok(f.app.state.sharedGifts.some(g=>g.id==='upgraded-1'));
  }
});
...
for(const amount of [3,5,10])void test(`upgrade ${amount} selects distinct gifts and consumes them without Stars`,async t=>{
  ...
  assert.ok(f.app.renderVals().upgradeRows.every(row=>row.chance==='8%'));
...
void test('batch source selection can toggle, preserves independent prices and rejects a too-cheap target',async t=>{
  ...
  assert.deepEqual(plain(f.app.renderVals().upgradeRows.map(row=>row.chance)),['8%','48%','8%']);
...
void test('same-owner refresh preserves upgrade selections and any ordinary canonical gift is a valid source',async t=>{
  ...
  assert.equal(f.app.chanceOf(),8);assert.match(f.app.renderVals().ctaNoteText,/Noct Stars не списываются/);
