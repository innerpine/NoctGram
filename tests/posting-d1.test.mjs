  ...
  async (t) => {
    ...
    await t.test(
      ...
      async () => {
        ...
        const reply = await api.reviewSpam({
          kind: 'comment',
          targetId: replyId,
          actorId: 'spam-alice',
          contextId: postId,
          payload: { text: 'unixgram.com reply', replyTo: commentId },
        });
        await api.spamModerationPost(
          'spamReview',
          { id: reply.id, decision: 'approve' },
          'spam-mod',
        );
        assert.equal(
          (await first('SELECT replyTo FROM comments WHERE id=?', replyId))
            .replyTo,
          commentId,
        );
        ...
        await run(
          "INSERT INTO uploads(id,userId,name,type,bytes,state,created) VALUES(?,'spam-alice','fixture.png','image/png',4,'ready',?)",
          fileId,
          Date.now(),
        );
        await run(
        ...
        const attached = await api.changeRoom('spam-alice', {
          action: 'send',
          id: room.id,
          key: randomUUID(),
          text: 'unixgram.com file',
          attachments: [fileId],
        });
        await api.spamModerationPost(
          'spamReview',
          { id: attached.id, decision: 'approve' },
          'spam-mod',
        );
        assert.equal(
          (await api.readRoom('spam-mod', room.id)).messages.at(-1)
            .attachments[0].id,
          fileId,
        );
      },
