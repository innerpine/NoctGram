...
test('direct reactions: all emoji, idempotency, replacement, counts and viewer ownership', async () => {
...
test('invalid reaction, outsider, wrong peer, hidden/deleted messages cannot be mutated', async () => {
...
test('direct reactions enforce bilateral blocks, message privacy and account restrictions', async () => {
...
test('direct auth changes at write time cannot add/remove reactions', async () => {
  beforeWrite = () =>
    sql.exec(
...
test('group reactions aggregate, replace, remove and survive refetch', async () => {
...
test('groups reject outsiders, wrong room, left/banned members and blocked senders', async () => {
...
test('groups repeat authorization at mutation time including same-remove requests', async () => {
  beforeWrite = () =>
    sql.exec(
...
test('secret chats never accept or return plaintext reactions', async () => {
...
test('deleting messages cleans reactions; local hide preserves peer reaction', async () => {
  await dm('bob', '👍');
  await dm('alice', '❤️');
...
test('foreign keys clean hard deletes; database rejects unknown emoji', async () => {
  await dm('bob', '👍');
  await group('bob', '👍');
  assert.throws(
    () =>
...
test('account removal cleans its reactions on surviving messages', async () => {
  await group('bob', '👍');
  await group('carol', '❤️');
  await dm('bob', '🔥');
  sql
...
test('reaction export only includes own accessible group data and rechecks every page', async () => {
  await group('bob', '👍');
  await group('carol', '🔥');
  const section = api
