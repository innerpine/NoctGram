...
async function home(period = '7', charts = '1', user = 'zz_chart_qa_me') {
  const response = await fetch(
    base +
      '/api/music?' +
      new URLSearchParams({ action: 'home', period, charts }),
    {
      headers: {
...
assert.equal(today.listeners.length, 30);
assert.ok(today.mine.rank > 30);
...
console.log(
  'Charts: today/week/month boundaries, own rank beyond top 30, lightweight home and invalid period passed.',
);
