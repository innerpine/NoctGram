...
const migrationIndex = journal.entries.find(entry => entry.tag === migrationTag)?.idx;
assert.ok(Number.isInteger(migrationIndex), 'Community migration must be in the journal');
...
test('migration backfills every completed person and uses a real administrator as owner', async () => {
...
test('empty installs wait for a real administrator and later grants backfill existing users', async () => {
  await withFixture(async (sqlite) => {
    migrate(sqlite);
    person(sqlite, 'local_seedy');
...
test('completed registrations enroll automatically; incomplete and excluded accounts do not', async () => {
  await withFixture(async (sqlite) => {
    migrate(sqlite);
...
test('enrollment events preserve leave, ban, archive, roles and read position', async () => {
  await withFixture(async (sqlite) => {
    person(sqlite, 'owner');
...
test('community rejoin and member additions exceed 200 while ordinary groups keep their cap', async () => {
  await withFixture(async (sqlite) => {
    person(sqlite, 'owner');
    grantAdmin(sqlite, 'owner');
    migrate(sqlite);
