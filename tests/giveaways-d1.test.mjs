...
test('giveaways use real D1 transactions, SQL limits, uniform membership ranks and one debit', { timeout: 180000 }, async (t) => {
  ...
  await d.prepare("INSERT INTO star_transfers(id,recipient,amount,kind,created) VALUES('grant','a',1000,'admin_grant',?)").bind(now).run();
  ...
  const concurrent = await Promise.allSettled([api.createGiveaway('a', draft, now), api.createGiveaway('a',{...draft,key:crypto.randomUUID()},now)]);
  assert.equal(concurrent.filter(r=>r.status==='fulfilled').length,1);
  const giveaway = concurrent.find(r=>r.status==='fulfilled').value.giveaway;
  assert.equal(giveaway.creator,'a');
  assert.equal((await d.prepare("SELECT sender FROM star_transfers WHERE kind='giveaway_debit'").first()).sender,'a');
  assert.equal((await d.prepare("SELECT sender FROM chat_room_messages WHERE giveawayId=?").bind(giveaway.id).first()).sender,'a');
  ...
  await d.prepare("INSERT INTO channel_members(channelId,userId,role,created) VALUES('channel','b','admin',?)").bind(now).run();
  await assert.rejects(api.createGiveaway('b',{...draft,key:crypto.randomUUID(),targetKind:'channel',targetId:'channel'},now), error => error.status===403);
