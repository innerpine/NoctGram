...
const root = process.cwd(),
  source = process.env.NOCT_GIVEAWAY_SOURCE || root;
const require = createRequire(join(root, 'package.json'));
const { Miniflare, convertV4MiniflareOptions } = require('miniflare');
const { build } = require('esbuild');

await test(
  'giveaways use real D1 transactions, SQL limits, uniform membership ranks and one debit',
  { timeout: 180000 },
  async (t) => {
    const mf = new Miniflare(
      convertV4MiniflareOptions({
        modules: true,
        script:
          'export default {fetch(){return new Response("Isolated giveaway test",{status:404})}};',
    ...
    const sqlFiles = JSON.parse(
      readFileSync(join(root, 'drizzle/meta/_journal.json'), 'utf8'),
    )
      .entries.filter((entry) => entry.tag !== '0044_giveaways')
      .map((entry) => join(root, 'drizzle', entry.tag + '.sql'));
    sqlFiles.push(join(source, 'drizzle/0044_giveaways.sql'));
    for (const file of sqlFiles)
      for (const sql of readFileSync(file, 'utf8').split(
        '--> statement-breakpoint',
      ))
        if (sql.trim()) await d.prepare(sql).run();
    globalThis.__giveawayRealD1 = d;
    t.after(() => {
      delete globalThis.__giveawayRealD1;
    });
    const compiled = await build({
      entryPoints: [join(source, 'lib/giveaways.ts')],
      bundle: true,
      write: false,
      format: 'esm',
      platform: 'node',
      plugins: [
        {
          ...
          setup(build) {
            ...
            build.onResolve({ filter: /^\.\// }, (args) => {
              try {
                readFileSync(join(args.resolveDir, args.path + '.ts'));
                return undefined;
              } catch {
                return { path: join(root, 'lib', args.path + '.ts') };
              }
            });
          },
        },
    ...
    await d
      .prepare(
        "INSERT INTO chat_rooms(id,ownerId,name,created,updatedAt) VALUES('group','owner','Group',?,?)",
      )
      .bind(now, now)
      .run();
    for (const id of ['owner', 'a', 'b', 'c'])
      await d
        .prepare(
          "INSERT INTO chat_room_members(roomId,userId,role,joinedAt) VALUES('group',?,?,?)",
        )
        .bind(id, id === 'owner' ? 'owner' : 'member', now)
        .run();
    await d
      .prepare(
    ...
    await d
      ...
      .bind(now)
      .run();
    const premium = await api.createGiveaway(
      'owner',
      {
        ...draft,
        key: crypto.randomUUID(),
    ...
    assert.equal(
      (
        await d
          .prepare("SELECT source FROM premium_entitlements WHERE userId='a'")
          .first()
      ).source,
      'giveaway',
    );
    assert.equal(
      (await api.getGiveaway('b', premium.giveaway.id, premium.giveaway.endsAt))
        .refund,
      500,
    );
  },
