...
await test('blocks direct and obfuscated domain forms without executing supplied code', () => {
  for (const input of [
    domain,
...
await test('allows unrelated text, harmless Base64, code and similar unlisted domains', () => {
  for (const input of [
    'Привет, как дела?',
    'unix grammar',
    'notunixgram.com',
...
await test('configured hyphenated labels do not produce exponential regex backtracking', () => {
...
await test('long content is bounded, and fingerprints preserve meaningful distinctions', () => {
  assert.equal(
