...
await test('current eligible members only; unfilled slots return funded stars', async () =>
  isolated(async (f) => {
    const { giveaway } = await api.createGiveaway(
      'owner',
...
await test('membership changes between COUNT and transaction retry against current set', async () =>
  isolated(async (f) => {
    const { giveaway } = await api.createGiveaway(
      'owner',
      f.body({ winnerCount: 3 }),
...
await test('Premium costs 500 per winner and extends existing paid expiration exactly once', async () =>
  isolated(async (f) => {
...
await test('insufficient funds and revocation at commit cannot debit or publish', async () =>
  isolated(async (f) => {
    await assert.rejects(
...
await test('transaction failure cannot leave a partial draw; next job recovers', async () =>
  isolated(async (f) => {
...
await test('blocked, deleted and incomplete accounts cannot win; empty target refunds all', async () =>
  isolated(async (f) => {
    const { giveaway } = await api.createGiveaway(
      'owner',
...
await test('committed receipt survives post-payment access loss and read-only retry', async () =>
  isolated(async (f) => {
    const draft = f.body();
