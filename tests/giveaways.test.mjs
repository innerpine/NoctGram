...
test('ordinary group members fund Stars and Premium from their own wallet', async () => {
  for (const prize of ['stars', 'premium']) {
    await isolated(async (f) => {
      ...
      const published = f.sql.prepare('SELECT sender FROM chat_room_messages WHERE giveawayId=?').get(result.giveaway.id);
      ...
      assert.equal(f.sql.prepare("SELECT COUNT(*) AS n FROM star_transfers WHERE kind='giveaway_debit'").get().n, 1);
      await api.settleGiveaway(result.giveaway.id, result.giveaway.endsAt);
      const completed = await api.getGiveaway('member', result.giveaway.id, result.giveaway.endsAt);
      assert.deepEqual(completed.winners.map((winner) => winner.id).sort(), ['admin', 'other', 'owner']);
      assert.equal(f.wallet('member'), 10000 - cost);
    });
  }
});

test('membership and account restrictions still prevent group giveaway creation', async () => {
  const scenarios = [
  ...
  for (const scenario of scenarios) {
    await isolated(async (f) => {
      f.sql.exec(scenario);
      await assert.rejects(api.createGiveaway('member', f.body(), f.now),
        (error) => error.status === 403 && error.code === 'GIVEAWAY_REJECTED');
      assert.equal(f.wallet('member'), 10000);
      assert.equal(f.sql.prepare('SELECT COUNT(*) AS n FROM giveaways').get().n, 0);
      assert.equal(f.sql.prepare('SELECT COUNT(*) AS n FROM chat_room_messages').get().n, 0);
    });
  }
});

test('demotion to ordinary member does not revoke group giveaway creation', async () =>
  isolated(async (f) => {
    f.beforeBatch((sql) => sql.exec("UPDATE chat_room_members SET role='member' WHERE userId='admin'"));
...
test('only the channel owner can create; administrators, editors and subscribers cannot', async () =>
  isolated(async (f) => {
    for (const actor of ['member', 'admin']) {
      for (const prize of ['stars', 'premium']) {
        await assert.rejects(
          api.createGiveaway(actor, f.body({ targetKind: 'channel', targetId: 'channel', prize }), f.now),
          (error) => error.status === 403 && /только его владелец/.test(error.message),
        );
      }
    }
    assert.equal(f.wallet('member'), 10000);
    assert.equal(f.wallet('admin'), 10000);
    assert.equal(f.sql.prepare('SELECT COUNT(*) AS n FROM giveaways').get().n, 0);
    assert.equal(f.sql.prepare('SELECT COUNT(*) AS n FROM posts WHERE giveawayId IS NOT NULL').get().n, 0);
    const { giveaway } = await api.createGiveaway(
      'owner',
...
test('channel ownership is rechecked inside the payment transaction', async () =>
  isolated(async (f) => {
    f.beforeBatch((sql) => sql.exec("UPDATE users SET ownerId='admin' WHERE id='channel'"));
    await assert.rejects(
      api.createGiveaway('owner', f.body({ targetKind: 'channel', targetId: 'channel' }), f.now),
      /Не удалось/,
    );
    assert.equal(f.wallet('owner'), 10000);
    assert.equal(f.wallet('admin'), 10000);
