...
const stubs = {
  ...
  '@/lib/rate-limit':
    'export async function rateLimit(...args){globalThis.__giveawayRoutes.calls.push(["rate",...args]);}',
  ...
  '@/lib/auth-session':
    'export function setting(key){return key==="NOCT_JOBS_SECRET"?globalThis.__giveawayRoutes.jobsSecret:undefined;}',
  '@/lib/notifications':
...
async function route(file) {
  const compiled = await build({
    entryPoints: [path.join(source, file)],
    bundle: true,
    write: false,
    platform: 'node',
    format: 'esm',
    plugins: [
      {
        ...
        setup(build) {
          ...
          build.onLoad(
            { filter: /.*/, namespace: 'route-fixture' },
            (args) => ({ contents: stubs[args.path], resolveDir: root }),
          );
        },
      },
    ],
...
function setup() {
  return (globalThis.__giveawayRoutes = {
    actor: 'owner',
    readOnly: false,
    blocked: false,
    committed: false,
    jobsSecret: 'isolated-test-secret',
    calls: [],
  });
}
const payload = () => ({
...
function request(body = payload(), headers = {}) {
  return new Request('https://noctgram.test/api/giveaways', {
    method: 'POST',
    headers: {
...
await test('cross-origin and cross-site requests fail before viewing or paying', async () => {
...
await test('matching authenticated actor delegates exact draft and returns private committed receipt', async () => {
  const s = setup(),
    body = payload();
  ...
  assert.deepEqual(await response.json(), {
    giveaway: { id: 'giveaway:receipt', creator: 'owner', totalCost: 500 },
    balance: 4500,
  });
});

await test('read-only account can resolve a committed retry; new creation remains backend-rejected', async () => {
  const s = setup();
  s.readOnly = true;
  s.committed = true;
  ...
  assert.equal(response.status, 200);
  assert.equal(
    s.calls.some(([name]) => name === 'writable'),
    false,
  );
...
await test('GET respects session and readable guard before loading a private giveaway', async () => {
  let s = setup();
  s.blocked = true;
  let response = await giveaways.GET(
    new Request('https://noctgram.test/api/giveaways?id=secret'),
  );
  assert.equal(response.status, 403);
  assert.equal(
    s.calls.some(([name]) => name === 'get'),
    false,
  );
  s = setup();
  s.actor = null;
  response = await giveaways.GET(
    new Request('https://noctgram.test/api/giveaways?id=secret'),
  );
  assert.equal(response.status, 401);
  assert.equal(
...
await test('jobs requires exact configured Bearer token and never acts on unauthorized calls', async () => {
  for (const [secret, authorization] of [
    ['isolated-test-secret', ''],
    ['isolated-test-secret', 'Bearer wrong'],
    ['', 'Bearer '],
  ]) {
    const s = setup();
    s.jobsSecret = secret;
    const response = await jobs.POST(
      new Request('https://noctgram.test/api/jobs/run?task=giveaways', {
        method: 'POST',
...
await test('authorized giveaway-only job settles prizes without sending push or other jobs', async () => {
  const s = setup();
  const response = await jobs.POST(
    new Request('https://noctgram.test/api/jobs/run?task=giveaways', {
      method: 'POST',
      headers: { authorization: 'Bearer isolated-test-secret' },
    }),
  );
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), {
...
await test('ordinary authorized jobs still settle prizes, calls, push and upload cleanup', async () => {
  const s = setup();
  const response = await jobs.POST(
    new Request('https://noctgram.test/api/jobs/run', {
      method: 'POST',
      headers: { authorization: 'Bearer isolated-test-secret' },
    }),
  );
