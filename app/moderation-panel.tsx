...
import { StaffSelect as Choice } from './staff-select';
      ...
      try {
        if (tab === 'users') {
          ...
          if (targetId) {
            ...
            setSelected((old) =>
              old ? page.people.find((p) => p.id === old.id) || old : null,
            );
          }
          setPeople((old) =>
            append
  ...
  return (
    <section className="moderation-panel">
      <div className="moderation-heading">
        ...
        <div>
          <h2>{canAdmin ? 'Кабинет команды' : 'Модерация'}</h2>
          <p>Аккаунты, обращения и безопасность NoctGram</p>
        </div>
        {(tab === 'users' || tab === 'appeals') && (
          <button
      ...
      >
            ...
            {selected && (
              ...
              >
                <div className="moderation-editor-title">
                  <div className="staff-target">
                    <Avatar person={selected} size={40} />
                  ...
                  <>
                    ...
                    <label>
                      <span className="staff-field-caption">
                        Причина решения <small>{reason.length}/500</small>
                      </span>
                      <textarea
                        value={reason}
                        ...
                        rows={3}
                        disabled={busy}
                        placeholder="Укажите нарушение и основание для решения"
                      />
                    </label>
                    <p className="account-note">
                ...
                >
                  ...
                  <em
                    className={
                      ...
                      (p.moderator ? 'team' : p.mode || 'active')
                    }
                  >
                    {p.moderator
                      ? 'Модератор'
                ...
                <article key={a.id}>
                  ...
                  {a.status === 'pending' ? (
                    <>
                      ...
                      <p className="account-note">
                        Принятие снимает оспариваемое ограничение. Если было
                        выдано новое, оно останется в силе.
                      </p>
                    </>
                  ) : (
        ...
        {loading && (tab === 'users' || tab === 'appeals') && (
          <output className="connections-status">Загружаем…</output>
        )}
      </Tabs>
    </section>
  );
